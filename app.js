...
const CACHE_KEY = 'stats-cov-root-cache-v53';
...
function excelCoreXml(exportedAt) {
  return `<?xml version="1.0" encoding="UTF-8" standalone="yes"?>
<cp:coreProperties xmlns:cp="http://schemas.openxmlformats.org/package/2006/metadata/core-properties" xmlns:dc="http://purl.org/dc/elements/1.1/" xmlns:dcterms="http://purl.org/dc/terms/" xmlns:dcmitype="http://purl.org/dc/dcmitype/" xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance"><dc:title>Stats COV Export</dc:title><dc:creator>Stats COV</dc:creator><cp:lastModifiedBy>Stats COV</cp:lastModifiedBy><dcterms:created xsi:type="dcterms:W3CDTF">${excelXmlEscape(exportedAt)}</dcterms:created><dcterms:modified xsi:type="dcterms:W3CDTF">${excelXmlEscape(exportedAt)}</dcterms:modified></cp:coreProperties>`;
}

function excelAppXml(sheets) {
  return `<?xml version="1.0" encoding="UTF-8" standalone="yes"?>
<Properties xmlns="http://schemas.openxmlformats.org/officeDocument/2006/extended-properties" xmlns:vt="http://schemas.openxmlformats.org/officeDocument/2006/docPropsVTypes"><Application>Stats COV</Application><DocSecurity>0</DocSecurity><ScaleCrop>false</ScaleCrop><HeadingPairs><vt:vector size="2" baseType="variant"><vt:variant><vt:lpstr>Worksheets</vt:lpstr></vt:variant><vt:variant><vt:i4>${sheets.length}</vt:i4></vt:variant></vt:vector></HeadingPairs><TitlesOfParts><vt:vector size="${sheets.length}" baseType="lpstr">${sheets.map((s) => `<vt:lpstr>${excelXmlEscape(excelSheetName(s.name))}</vt:lpstr>`).join('')}</vt:vector></TitlesOfParts><Company>Stats COV</Company></Properties>`;
}
...
function exportTableRows() {
  ...
  const ballon = [...ballonOrPlayers()].sort((a, b) => Number(a.ballonOrRank || 999) - Number(b.ballonOrRank || 999));
  ...
  const summaryRows = [
    ...
    { indicateur: 'Joueurs classement', valeur: ranked.length },
    { indicateur: 'Match suivant', valeur: season?.draftMatch?.dateLabel || '' },
    { indicateur: 'Règle Ballon d’Or', valeur: '70% /Journée + 30% /match harmonisé' },
    { indicateur: 'Règle note générale', valeur: '30% /Journée + 70% /match harmonisé' },
  ];
  const rankingRows = ranked.map((player) => ({
    rang: Number(player.classement || 0), joueur: player.displayName, poste: getPlayerPositions(player).join(', '), j: Number(player.matchesPlayed || 0), pts: Number(player.points || 0), v: Number(player.wins || 0), n: Number(player.draws || 0), d: Number(player.losses || 0), bp: Number(player.goalsFor || 0), bc: Number(player.goalsAgainst || 0), diff: Number(player.goalDifference || 0), buts: Number(player.goals || 0), passes: Number(player.assists || 0), mvp: Number(player.mvps || 0), note: Number(playerOverallRating(player.playerId).rating || 0)
  }));
  const ballonRows = ballon.map((player) => ({
    rang: Number(player.ballonOrRank || 0), joueur: player.displayName, score: Number(player.ballonOrScore || 0), journee: Number(player.ballonOrSeasonScore || 0), harmonise: Number(player.ballonOrHarmonizedScore || 0), j: Number(player.matchesPlayed || 0), pts: Number(player.points || 0), ba: Number(player.goals || 0) + Number(player.assists || 0), mvp: Number(player.mvps || 0), classement: Number(player.classement || 0)
  }));
  const playerRows = players().map((player) => ({
    id: player.playerId, joueur: player.displayName, actif: isPlayerArchived(player) ? 'Non' : 'Oui', classement: isPlayerExcludedFromRanking(player) ? 'Non' : 'Oui', postes: getPlayerPositions(player).join(', '), avatar: player.avatarId || '', temporaire: player.temporaryMatchOnly ? 'Oui' : 'Non', note: playerOverallRating(player.playerId).rating
  }));
  const matchRows = completed.map((match, index) => ({
    journee: index + 1, date: match.dateLabel || match.dateIso || '', score: `${Number(match.yellowScore || 0)} - ${Number(match.blueScore || 0)}`, jaune: (match.teamYellowPlayerIds || []).map(playerName).join(', '), bleu: (match.teamBluePlayerIds || []).map(playerName).join(', '), mvpJaune: playerName(match.yellowMvpPlayerId), mvpBleu: playerName(match.blueMvpPlayerId)
  }));
  const contributionRows = [];
  completed.forEach((match, index) => {
    const rows = Array.isArray(match.contributions) && match.contributions.length ? match.contributions : buildMatchContributions(match);
    rows.filter((row) => Number(row.matchesPlayed || 0) > 0 || Number(row.goals || 0) || Number(row.assists || 0) || Number(row.mvps || 0)).forEach((row) => {
      contributionRows.push({ journee: index + 1, date: match.dateLabel || match.dateIso || '', joueur: playerName(row.playerId), pts: Number(row.points || 0), buts: Number(row.goals || 0), passes: Number(row.assists || 0), mvp: Number(row.mvps || 0), bp: Number(row.goalsFor || 0), bc: Number(row.goalsAgainst || 0), resultat: Number(row.wins || 0) ? 'V' : Number(row.draws || 0) ? 'N' : Number(row.losses || 0) ? 'D' : '' });
    });
  });
  const slots = season?.publishedEleven?.slots || {};
  const elevenRows = Object.entries(slots).filter(([, id]) => id).map(([slot, id]) => ({ poste: slot, joueur: playerName(id), note: playerOverallRating(id).rating }));
  (season?.publishedEleven?.benchPlayerIds || []).forEach((id) => elevenRows.push({ poste: 'Banc', joueur: playerName(id), note: playerOverallRating(id).rating }));
  return { summaryRows, rankingRows, ballonRows, playerRows, matchRows, contributionRows, elevenRows };
}
...
function createExcelExportBlob(rootData, exportedAt) {
  ...
  const sheets = [
    { name: 'Synthèse', title: 'Stats COV · Synthèse', subtitle, columns: [{ key: 'indicateur', label: 'Indicateur', width: 28 }, { key: 'valeur', label: 'Valeur', width: 58 }], rows: data.summaryRows },
    { name: 'Classement', title: 'Classement général', subtitle, columns: [{ key: 'rang', label: '#', width: 7, type: 'number' }, { key: 'joueur', label: 'Joueur', width: 22 }, { key: 'poste', label: 'Postes', width: 22 }, { key: 'j', label: 'J', width: 7, type: 'number' }, { key: 'pts', label: 'PTS', width: 8, type: 'number' }, { key: 'v', label: 'V', width: 6, type: 'number' }, { key: 'n', label: 'N', width: 6, type: 'number' }, { key: 'd', label: 'D', width: 6, type: 'number' }, { key: 'diff', label: '+/-', width: 8, type: 'number' }, { key: 'buts', label: 'Buts', width: 8, type: 'number' }, { key: 'passes', label: 'Passes', width: 8, type: 'number' }, { key: 'mvp', label: 'MVP', width: 8, type: 'number' }, { key: 'note', label: 'Note', width: 8, type: 'number' }], rows: data.rankingRows },
    { name: 'Ballon d Or', title: 'Ballon d’Or', subtitle: '70% contribution saison + 30% impact harmonisé', columns: [{ key: 'rang', label: '#', width: 7, type: 'number' }, { key: 'joueur', label: 'Joueur', width: 22 }, { key: 'score', label: 'Score', width: 9, type: 'number' }, { key: 'journee', label: '/Journée', width: 11, type: 'number' }, { key: 'harmonise', label: 'Harmonisé', width: 12, type: 'number' }, { key: 'j', label: 'J', width: 7, type: 'number' }, { key: 'pts', label: 'PTS', width: 8, type: 'number' }, { key: 'ba', label: 'B+A', width: 8, type: 'number' }, { key: 'mvp', label: 'MVP', width: 8, type: 'number' }, { key: 'classement', label: 'Gén.', width: 8, type: 'number' }], rows: data.ballonRows },
    { name: 'Joueurs', title: 'Joueurs', subtitle, columns: [{ key: 'id', label: 'ID', width: 22 }, { key: 'joueur', label: 'Joueur', width: 22 }, { key: 'actif', label: 'Actif', width: 10 }, { key: 'classement', label: 'Classement', width: 14 }, { key: 'postes', label: 'Postes', width: 26 }, { key: 'avatar', label: 'Avatar', width: 18 }, { key: 'temporaire', label: 'Temporaire', width: 12 }, { key: 'note', label: 'Note', width: 8, type: 'number' }], rows: data.playerRows },
    { name: 'Matchs', title: 'Matchs', subtitle, columns: [{ key: 'journee', label: 'Journée', width: 10, type: 'number' }, { key: 'date', label: 'Date', width: 14 }, { key: 'score', label: 'Score', width: 10 }, { key: 'jaune', label: 'Équipe jaune', width: 60 }, { key: 'bleu', label: 'Équipe bleue', width: 60 }, { key: 'mvpJaune', label: 'MVP jaune', width: 18 }, { key: 'mvpBleu', label: 'MVP bleu', width: 18 }], rows: data.matchRows },
    { name: 'Contributions', title: 'Contributions par match', subtitle, columns: [{ key: 'journee', label: 'Journée', width: 10, type: 'number' }, { key: 'date', label: 'Date', width: 14 }, { key: 'joueur', label: 'Joueur', width: 22 }, { key: 'pts', label: 'PTS', width: 8, type: 'number' }, { key: 'buts', label: 'Buts', width: 8, type: 'number' }, { key: 'passes', label: 'Passes', width: 8, type: 'number' }, { key: 'mvp', label: 'MVP', width: 8, type: 'number' }, { key: 'bp', label: 'BP', width: 8, type: 'number' }, { key: 'bc', label: 'BC', width: 8, type: 'number' }, { key: 'resultat', label: 'Rés.', width: 8 }], rows: data.contributionRows },
    { name: 'Onze type', title: 'Onze type', subtitle, columns: [{ key: 'poste', label: 'Poste', width: 12 }, { key: 'joueur', label: 'Joueur', width: 22 }, { key: 'note', label: 'Note', width: 8, type: 'number' }], rows: data.elevenRows },
  ];
  const entries = [
...
function createDefaultPlayer(displayName, avatarFamily = '1') {
...
function createEmptyDraftMatch(playersList = []) {
  ...
  return {
    ...
    time: nextInfo.time,
    presences: playersList.map((player) => ({ playerId: player.playerId, present: false })),
    teamYellowPlayerIds: [],
    teamBluePlayerIds: [],
...
function recentFormDotsMarkup(playerId, limit = 5) {
  ...
  return `<span class="form-dots" aria-label="Forme récente">${form.map((item) => `<span class="form-dot ${item.result === 'V' ? 'win' : item.result === 'D' ? 'loss' : 'draw'}" title="${item.result}"></span>`).join('')}</span>`;
}
...
function updateHeader() {
  ...
  if (state.activeView === 'home') {
    ...
    left = canShareGeneral() ? shareButtonMarkup('data-share-ranking="1"') : '';
  ...
  if (els.seasonBtn) {
    ...
    els.seasonBtn.classList.add('season-chip-attention');
  }
...
function ensureCoachEditors() {
  ...
  if (!state.coachPresenceEdits) {
    const map = {};
    (draft.presences || []).forEach((item) => { map[item.playerId] = !!item.present; });
    playersInMatch(draft).forEach((player) => { if (!(player.playerId in map)) map[player.playerId] = false; });
...
function draftPresenceMap() {
  ensureCoachEditors();
  const map = new Map();
  Object.entries(state.coachPresenceEdits || {}).forEach(([id, present]) => map.set(id, !!present));
  return map;
}
...
function createOrUpdateUpcomingMatch(nextDraft) {
  ...
  const presences = Array.isArray(nextDraft.presences) ? nextDraft.presences.filter((row) => activePlayers().some((player) => player.playerId === row.playerId)) : activePlayers().map((player) => ({ playerId: player.playerId, present: false }));
...
async function closeCurrentMatch(match) {
  ...
  const nextDraft = {
    ...
    dateLabel: nextInfo.date,
    time: nextInfo.time,
    presences: activePlayers(nextPlayers).map((player) => ({ playerId: player.playerId, present: false })),
    teamYellowPlayerIds: [],
    teamBluePlayerIds: [],
    yellowLines: { gb: [], dc: [], mc: [], bu: [] },
    blueLines: { gb: [], dc: [], mc: [], bu: [] },
...
function familyPoolForRating(pool, family, seasonId = state.activeSeasonId) {
  ...
  const filtered = source.filter((candidate) => ratingFamiliesForPlayer(candidate, seasonId).some((item) => item.family === family));
  return filtered.length ? filtered : source;
}

function playerImpactScoreForFamily(player, family, pool = [], seasonId = state.activeSeasonId, basis = 'match') {
  const played = playerMatchesForRating(player);
  if (!played) return 50;
  const familyPool = familyPoolForRating(pool, family, seasonId);
...
function playerSeasonPerformanceScore(player, pool = [], seasonId = state.activeSeasonId, basis = 'match') {
  const played = playerMatchesForRating(player);
  if (!played) return 50;
  const families = ratingFamiliesForPlayer(player, seasonId);
  const weighted = families.reduce((sum, item) => sum + playerImpactScoreForFamily(player, item.family, pool, seasonId, basis) * Number(item.weight || 0), 0);
  return clampNumber(Math.round(weighted), 0, 100);
}

function playerHarmonizedMatchScore(player, pool = [], seasonId = state.activeSeasonId) {
  const matchScore = playerSeasonPerformanceScore(player, pool, seasonId, 'match');
  const played = playerMatchesForRating(player);
  const confidence = played / (played + 3);
  return clampNumber(Math.round(matchScore * confidence + 50 * (1 - confidence)), 0, 100);
}

function playerHybridPerformanceScore(player, pool = [], seasonId = state.activeSeasonId, seasonWeight = 0.3) {
  const seasonScore = playerSeasonPerformanceScore(player, pool, seasonId, 'journee');
  const harmonized = playerHarmonizedMatchScore(player, pool, seasonId);
...
function playerBallonOrPerformanceScore(player, pool = [], seasonId = state.activeSeasonId) {
  return playerHybridPerformanceScore(player, pool, seasonId, 0.7);
}
...
function playerOverallRating(playerId, seasonId = state.activeSeasonId, memo = new Map()) {
  ...
  const adjustment = Math.round(clampNumber((seasonTarget - base) * 0.65, -12, 12));
...
function playerOverallBadgeMarkup(player) {
  ...
  return `
    <div class="player-overall-badge ${positive ? 'positive' : 'negative'}" title="Note dynamique saison">
...
function buildElevenOfSeason() {
  ...
  const availableGoalkeepers = allPlayers.filter((player) => playerRoles(player.playerId).includes('GB'));
  const goalkeeper = [...(availableGoalkeepers.length ? availableGoalkeepers : allPlayers)]
    .sort((a, b) => playerScoreForFamily(b, 'gk') - playerScoreForFamily(a, 'gk') || normalizeString(a.displayName).localeCompare(normalizeString(b.displayName)))[0];

  const remaining = allPlayers
    .filter((player) => player.playerId !== goalkeeper?.playerId)
    .sort((a, b) => playerOverallScore(b) - playerOverallScore(a) || normalizeString(a.displayName).localeCompare(normalizeString(b.displayName)));

  const selected = [goalkeeper, ...remaining.slice(0, 10)].filter(Boolean);
  const selectedById = new Map(selected.map((player) => [player.playerId, player]));
  const slots = emptyDetailedSlots();
  if (goalkeeper) slots.GB = goalkeeper.playerId;

  const occupied = new Set(goalkeeper ? [goalkeeper.playerId] : []);
  const fieldOrder = ['DC','MCG','MCD','MOC','AG','AD','ACG','ACD','DG','DD','MG','MD','DCG','DCD','MDCG','MDCD','BU'];

  const rankCandidates = (player, role) => {
    const roles = playerRoles(player.playerId);
    const roleIndex = roles.indexOf(role);
    const family = roleFamily(role);
    const affinity = roleIndex === -1 ? -25 : (30 - roleIndex * 4);
    return playerOverallScore(player) + playerScoreForFamily(player, family) * 0.35 + affinity;
  };

  for (const role of POSITION_OPTIONS.filter((role) => role !== 'GB')) {
    const candidates = [...selectedById.values()]
      .filter((player) => !occupied.has(player.playerId) && playerRoles(player.playerId).includes(role))
      .sort((a, b) => rankCandidates(b, role) - rankCandidates(a, role) || normalizeString(a.displayName).localeCompare(normalizeString(b.displayName)));
    for (const candidate of candidates) {
      const slot = bestSlotCandidatesForRole(role).find((name) => !slots[name]);
      if (!slot) continue;
      slots[slot] = candidate.playerId;
      occupied.add(candidate.playerId);
      break;
    }
  }

  const leftovers = [...selectedById.values()]
    .filter((player) => !occupied.has(player.playerId))
    .sort((a, b) => playerOverallScore(b) - playerOverallScore(a) || normalizeString(a.displayName).localeCompare(normalizeString(b.displayName)));
  for (const player of leftovers) {
    const freeSlot = fieldOrder.find((slot) => !slots[slot]);
    if (!freeSlot) break;
    slots[freeSlot] = player.playerId;
...
function renderElevenManage() {
  ...
  const available = allPlayers.filter((player) => !assigned.has(player.playerId) && !benched.has(player.playerId));
  return `
...
async function shareTrophyDetail(trophyId) {
  ...
  if (!blob) return;
  await shareBlob(blob, `stats-cov-trophee-${normalizeString(player.displayName)}-${seasonLabelById(trophy.seasonId).replace('/', '-')}.png`, 'Stats COV', `${trophy.title} - ${player.displayName}`);
}
...
function featureDialogMarkup(kind) {
  ...
  return `
    <section class="records-grid">
      <article class="card"><div class="card-head centered-head"><h3>Records saison</h3></div><div class="card-body stats-table">
        ${statsRow('Meilleur buteur', `${records.bestScorerSeason?.displayName || '—'} · ${records.bestScorerSeason?.goals || 0}`)}
        ${statsRow('Meilleur passeur', `${records.bestAssistSeason?.displayName || '—'} · ${records.bestAssistSeason?.assists || 0}`)}
        ${statsRow('Roi des MVP', `${records.bestMvpSeason?.displayName || '—'} · ${records.bestMvpSeason?.mvps || 0}`)}
        ${statsRow('Meilleure présence', `${records.bestPresenceSeason?.displayName || '—'} · ${records.bestPresenceSeason?.presences || 0}`)}
        ${statsRow('Série présence actuelle', `${playerDisplayName(records.bestPresenceStreak?.playerId) || '—'} · ${records.bestPresenceStreak?.value || 0}`)}
      </div></article>
...
function computeSummary() {
  ...
  return {
    ...
    moyennePresent: allPlayers.length ? Math.round(allPlayers.reduce((sum, p) => sum + Number(p.presences || 0), 0) / allPlayers.length) : 0
  };
}
    ...
    <div class="player-showcase">
      ...
      <section class="card">
        ...
        <div class="card-body impact-split-grid">
          <section class="impact-side yellow-soft">
          ...
          <section class="impact-side blue-soft">
...
function openPlayer(playerId) {
  ...
  state.playerPositionsEditId = null;
  state.playerPositionsDraft = [];
  els.playerDialogTitle.innerHTML = canShareGeneral() ? shareButtonMarkup('data-share-player="1"') : '';
  els.playerDialogBody.innerHTML = playerDetailMarkup(player);
  attachPlayerDialogInteractions();
...
function openFeature(kind) {
  const title = kind === 'eleven' ? 'Onze type' : 'Records';
...
function ballonOrPlayers() {
  ...
  const ranked = source.map((player) => {
    const seasonScore = playerSeasonPerformanceScore(player, source, state.activeSeasonId, 'journee');
    const matchScore = playerSeasonPerformanceScore(player, source, state.activeSeasonId, 'match');
    const harmonized = playerHarmonizedMatchScore(player, source, state.activeSeasonId);
...
function renderBallonOr() {
  ...
  return `
    <section class="ballon-page ${isWinner ? 'winner' : ''}">
      ...
      <div class="ballon-auto-label">Défilement automatique · ${isWinner ? '10' : Number(player.ballonOrRank || 0) <= 5 ? '6' : '3'} sec / joueur</div>
      ...
      <div class="ballon-stats-grid">
        ...
        <div><span>/Journée</span><strong>${Number(player.ballonOrSeasonScore || 0)}</strong></div>
        <div><span>Harmonisé</span><strong>${Number(player.ballonOrHarmonizedScore || 0)}</strong></div>
      </div>
...
function scheduleBallonOrAutoAdvance() {
  ...
  const delay = rank === 1 ? 10000 : rank <= 5 ? 6000 : 3000;
...
async function createBallonOrRankingShareBlob(range) {
  ...
  ctx.font = '800 34px Inter, Arial';
  ctx.textAlign = 'left';
  ctx.fillText(`Saison ${currentSeasonDisplay()}`, 220, 84);
  ctx.fillStyle = '#ffffff';
  ctx.font = '950 58px Inter, Arial';
  ctx.fillText(ballonOrRangeTitle(range).toUpperCase(), 220, 150);
  ctx.fillStyle = 'rgba(255,255,255,.7)';
  ctx.font = '700 26px Inter, Arial';
  ctx.fillText('Classement calculé avec note joueur, points, impact collectif, MVP, fiabilité et présence.', 54, 226);
...
async function shareBallonOrRanking(range) {
  ...
  if (!blob) return;
  const label = range === 'podium' ? 'podium' : range === 'ten4' ? '10-4' : 'derniere-11';
  await shareBlob(blob, `stats-cov-ballon-or-${label}-${currentSeasonDisplay().replace('/', '-')}.png`, 'Stats COV', ballonOrRangeTitle(range));
}
...
function renderHome() {
  ...
  return `
    ...
    <section class="card hero-card next-match-card">
      <div class="hero-grid hero-center next-match-home-grid">
        <div>
          ...
          ${canManage ? `<div class="match-day-picker"><input class="match-date-input" data-match-date-input="1" type="date" value="${nextMatch.dateIso || ''}" /></div>` : ''}
        </div>
        ${canManage ? `
          <div class="home-actions-scroll">
            <button class="primary-btn" data-view-switch="completeMatch" type="button">Faire compo</button>
            <button class="secondary-btn" data-save-match-day="1" type="button">Jour de match</button>
    ...
    <section class="card">
      <div class="card-head"><h2>Résumé saison</h2></div>
      <div class="card-body summary-grid">
    ...
    <section class="section-stack">
      ${leaderSection('Leaders classement', topPlayers('points'))}
      ${leaderSection('Leaders buteurs', topPlayers('goals'))}
      ${leaderSection('Leaders passeurs', topPlayers('assists'))}
    </section>

    <section class="card">
      <div class="card-head"><h2>Onze type</h2></div>
      <div class="card-body action-grid">
        <button class="feature-btn" data-open-feature="eleven" type="button">Consulter le onze type</button>
      </div>
    </section>

    <section class="card">
      <div class="card-head"><h2>Explorer</h2></div>
      <div class="card-body action-grid">
        <button class="feature-btn" data-open-feature="records" type="button">Records</button>
      </div>
    </section>
...
async function saveAdminCodes() {
  ...
  if (!coachCode || !presidentCode) {
    alert('Renseigne les deux codes.');
    return;
  }
...
async function addAdminPlayer() {
  ...
  const newPlayer = {
    ...createDefaultPlayer(name, variant),
  ...
  const nextSeason = {
    ...
    draftMatch: season?.draftMatch ? (() => {
      const previousPresence = new Map((season.draftMatch.presences || []).map((row) => [row.playerId, !!row.present]));
      return {
        ...season.draftMatch,
        presences: activePlayers(nextPlayers).map((player) => ({ playerId: player.playerId, present: previousPresence.get(player.playerId) || false }))
      };
...
async function deleteAdminPlayer(playerId) {
  if (!confirm('Retirer ce joueur des prochaines compos ? Ses anciens matchs et ses stats restent conservés.')) return;
  const season = seasonData();
...
async function addAdminTrophy() {
  ...
  if (!winnerPlayerId) {
    alert('Choisis le gagnant du trophée.');
    return;
  }
...
async function addAdminSeason() {
  ...
  if (!displayName) {
    alert('Entre un nom de saison.');
    return;
  }
  const seasonId = seasonDisplayToId(displayName);
  if (!seasonId) {
    alert('Nom de saison invalide.');
    return;
  }
...
async function closeActiveSeasonAndCreateNext() {
  ...
  if (!nextDisplay) {
    alert('Impossible de calculer la prochaine saison.');
    return;
  }
...
function renderPresenceEditor(target) {
  ...
  const presentCount = [...map.values()].filter(Boolean).length;
  return `
    <section class="card">
      <div class="card-head"><h2>Total présents : ${presentCount}</h2></div>
      <div class="card-body coach-presence-list">
        ${list.map((player) => {
          const present = map.get(player.playerId) || false;
          return `
            <div class="presence-row">
              <img class="presence-avatar" src="${avatarFor(player)}" alt="${escapeHtml(player.displayName)}" />
              <div class="presence-name">${escapeHtml(player.displayName)}</div>
              <button class="presence-toggle ${present ? 'present' : 'absent'}" data-toggle-presence="${player.playerId}" type="button">${present ? 'Présent' : 'Absent'}</button>
            </div>
          `;
        }).join('')}
      </div>
...
async function createRankingShareBlob() {
  ...
  ctx.fillText(`Classement complet · Saison ${currentSeasonDisplay()}`, 200, 90);
  ctx.fillStyle = '#ffffff';
  ctx.font = '900 62px Inter, Arial';
  ctx.fillText('Vincennes COV', 200, 145);
...
async function shareMany(blobs, filenameBase, title, text) {
  const files = blobs.map((blob, index) => new File([blob], `${filenameBase}-${index === 0 ? 'terrain' : 'sobre'}.png`, { type: 'image/png' }));
  try {
...
async function shareMatch(matchId) {
  ...
  await shareBlob(blob, `stats-cov-${match.dateLabel.replaceAll('/', '-')}.png`, 'Stats COV', `Match du ${match.dateLabel}`);
}

async function shareMatchBundle(matchId) {
  await shareMatch(matchId);
}

async function shareRanking() {
  const blob = await createRankingShareBlob();
  await shareBlob(blob, `stats-cov-classement-${currentSeasonDisplay().replace('/', '-')}.png`, 'Stats COV', `Classement complet ${currentSeasonDisplay()}`);
}
...
async function createSingleTeamCompoShareBlob(source, teamKey = 'yellow') {
  ...
  ctx.fillText(`COMPO ${label}`, 600, 292);
...
async function createCompoShareItems(source) {
  ...
  if (yellowCount && blueCount) {
    ...
    return [
      yellowBlob ? { blob: yellowBlob, filename: `stats-cov-compo-jaune-${datePart}.png` } : null,
      blueBlob ? { blob: blueBlob, filename: `stats-cov-compo-bleu-${datePart}.png` } : null
    ].filter(Boolean);
  }
  if (blueCount) {
    const blueBlob = await createSingleTeamCompoShareBlob(source, 'blue');
    return blueBlob ? [{ blob: blueBlob, filename: `stats-cov-compo-bleu-${datePart}.png` }] : [];
  }
  const blob = await createCompoShareBlob(source);
  return blob ? [{ blob, filename: `stats-cov-compo-${datePart}.png` }] : [];
}
...
async function createCompoShareBlob(source) {
  ...
  ctx.fillStyle = '#ffffff';
  ctx.font = '900 58px Inter, Arial';
  ctx.fillText(hasBlueTeam ? 'COMPO DU MATCH' : 'ONZE TYPE', 800, 280);
  ctx.fillStyle = '#ffd55c';
  ctx.font = '800 34px Inter, Arial';
  ctx.fillText(`Saison ${currentSeasonDisplay()}`, 800, 328);
  if (hasBlueTeam) {
    ctx.fillStyle = 'rgba(255,255,255,.82)';
    ctx.font = '700 28px Inter, Arial';
    ctx.fillText(source.dateLabel || currentSeasonDisplay(), 800, 370);
  }
...
async function shareCurrentCompo() {
  ...
  if (items.length === 1) await shareBlob(items[0].blob, items[0].filename, 'Stats COV', 'Compo du match');
  else await shareNamedBlobs(items, 'Stats COV', 'Compo du match');
}
...
async function shareCompoFromSource(sourceKey = 'current') {
  ...
  const items = await createCompoShareItems(source);
  if (!items.length) return;
  if (items.length === 1) await shareBlob(items[0].blob, items[0].filename, 'Stats COV', sourceKey === 'eleven' || sourceKey === 'eleven_editor' ? 'Onze type' : 'Compo du match');
  else await shareNamedBlobs(items, 'Stats COV', 'Compo du match');
}
...
async function sharePlayer(playerId) {
  const player = playerById(playerId);
  if (!player) return;
  const blob = await createPlayerShareBlob(player);
  await shareBlob(blob, `stats-cov-${normalizeString(player.displayName)}-${currentSeasonDisplay().replace('/', '-')}.png`, 'Stats COV', `Fiche joueur ${player.displayName}`);
}
...
function attachCoachInteractions() {
  ...
  els.viewMount.querySelectorAll('[data-toggle-presence]').forEach((btn) => btn.addEventListener('click', () => {
    ...
    state.coachPresenceEdits[id] = !state.coachPresenceEdits[id];
    if (!state.coachPresenceEdits[id]) {
  ...
  els.viewMount.querySelectorAll('[data-save-presence]').forEach((btn) => btn.addEventListener('click', async () => {
    const updated = playersInMatch(target).sort((a,b)=>normalizeString(a.displayName).localeCompare(normalizeString(b.displayName))).map((player) => ({ playerId: player.playerId, present: !!state.coachPresenceEdits[player.playerId] }));
...
els.seasonBtn.addEventListener('click', () => els.seasonDialog.showModal());
